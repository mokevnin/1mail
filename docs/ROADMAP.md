...
| 6 | Analytics + deliverability | 🟡 In progress | 1, 4 | Dashboards (aggregates over campaigns/automations) **✅ done** — workspace analytics overview (contacts/email-engagement/automations KPIs + send-cohort engagement time series via @mantine/charts). Still ⬜: sending domains + DKIM/SPF, suppression/bounce handling, A/B. |

...
| 6 | Analytics + deliverability | 🟡 In progress | 1, 4 | Dashboards **✅** (workspace analytics overview). Suppression list **✅** — central per-workspace do-not-send registry (unsubscribe/bounce/complaint/manual), fed by an events subscriber + manual CRUD, consulted in the send loop. Still ⬜: sending domains + DKIM/SPF, bounce/complaint **ingestion** (SES SNS / SMTP DSN → suppression), A/B. |

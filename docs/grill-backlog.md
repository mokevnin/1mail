...
| AI/MCP surface — server | **mark3labs/mcp-go** | Go lib | Chosen. Mature, widely-used Go MCP server lib (tools + prompts + resources). Mount as a 4th surface next to the ogen servers; auth rides the existing scoped API-token model. Grill generated-vs-projected before adopting. |

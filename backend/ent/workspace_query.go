...
type WorkspaceQuery struct {
	...
	withMemberships *WorkspaceMembershipQuery
...
func (_q *WorkspaceQuery) QueryMemberships() *WorkspaceMembershipQuery {
	query := (&WorkspaceMembershipClient{config: _q.config}).Query()
...
func (_q *WorkspaceQuery) WithMemberships(opts ...func(*WorkspaceMembershipQuery)) *WorkspaceQuery {
	query := (&WorkspaceMembershipClient{config: _q.config}).Query()
...
func (_q *WorkspaceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workspace, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *Workspace) { n.Edges.Memberships = []*WorkspaceMembership{} },
			func(n *Workspace, e *WorkspaceMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *WorkspaceQuery) loadMemberships(ctx context.Context, query *WorkspaceMembershipQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *WorkspaceMembership)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workspacemembership.FieldWorkspaceID)
	}
	query.Where(predicate.WorkspaceMembership(func(s *sql.Selector) {

...
func (_q *ProjectQuery) QueryContacts() *ContactQuery {
...
func (_q *ProjectQuery) QueryTrackingProfiles() *TrackingProfileQuery {
...
func (_q *ProjectQuery) WithContacts(opts ...func(*ContactQuery)) *ProjectQuery {
	query := (&ContactClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) WithTrackingProfiles(opts ...func(*TrackingProfileQuery)) *ProjectQuery {
	query := (&TrackingProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withContacts; query != nil {
		if err := _q.loadContacts(ctx, query, nodes,
			func(n *Project) { n.Edges.Contacts = []*Contact{} },
			func(n *Project, e *Contact) { n.Edges.Contacts = append(n.Edges.Contacts, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withTrackingProfiles; query != nil {
		if err := _q.loadTrackingProfiles(ctx, query, nodes,
			func(n *Project) { n.Edges.TrackingProfiles = []*TrackingProfile{} },
			func(n *Project, e *TrackingProfile) { n.Edges.TrackingProfiles = append(n.Edges.TrackingProfiles, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadContacts(ctx context.Context, query *ContactQuery, nodes []*Project, init func(*Project), assign func(*Project, *Contact)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(contact.FieldWorkspaceProjectID)
	}
	query.Where(predicate.Contact(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.ContactsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.WorkspaceProjectID
		if fk == nil {
			return fmt.Errorf(`foreign-key "workspace_project_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workspace_project_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ProjectQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Project, init func(*Project), assign func(*Project, *Event)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(event.FieldWorkspaceProjectID)
	}
...
func (_q *ProjectQuery) loadTrackingProfiles(ctx context.Context, query *TrackingProfileQuery, nodes []*Project, init func(*Project), assign func(*Project, *TrackingProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(trackingprofile.FieldWorkspaceProjectID)
	}
	query.Where(predicate.TrackingProfile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.TrackingProfilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ProjectQuery) loadAPITokens(ctx context.Context, query *ApiTokenQuery, nodes []*Project, init func(*Project), assign func(*Project, *ApiToken)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apitoken.FieldWorkspaceProjectID)
	}

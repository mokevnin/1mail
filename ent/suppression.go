...
// SuppressionEdges holds the relations/edges for other nodes in the graph.
type SuppressionEdges struct {
...
func (e SuppressionEdges) WorkspaceOrErr() (*Workspace, error) {
	if e.Workspace != nil {
	...
	for i := range columns {
		...
		case suppression.FieldReason:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Reason = suppression.Reason(value.String)
			}
...
// QueryWorkspace queries the "workspace" edge of the Suppression entity.
func (_m *Suppression) QueryWorkspace() *WorkspaceQuery {
	return NewSuppressionClient(_m.config).QueryWorkspace(_m)
}

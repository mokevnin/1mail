...
func (_q *WorkspaceQuery) QueryContacts() *ContactQuery {
...
func (_q *WorkspaceQuery) QueryEvents() *EventQuery {
...
func (_q *WorkspaceQuery) WithContacts(opts ...func(*ContactQuery)) *WorkspaceQuery {
	query := (&ContactClient{config: _q.config}).Query()
...
func (_q *WorkspaceQuery) WithEvents(opts ...func(*EventQuery)) *WorkspaceQuery {
	query := (&EventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WorkspaceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workspace, error) {
	...
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Workspace) { n.Edges.Events = []*Event{} },
			func(n *Workspace, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *WorkspaceQuery) loadContacts(ctx context.Context, query *ContactQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *Contact)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(contact.FieldWorkspaceID)
	}
...
func (_q *WorkspaceQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *Event)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Workspace)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(event.FieldWorkspaceID)
	}
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workspace.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *WorkspaceQuery) loadAPITokens(ctx context.Context, query *ApiTokenQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *ApiToken)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apitoken.FieldWorkspaceID)
	}

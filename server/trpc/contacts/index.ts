...
import { throwTrpcError, trpcGuard } from '#/lib/trpc.ts'
...
type ContactsRouterDependencies = {
  t: typeof import('#/lib/trpc-router.ts').t
}

export function createContactsRouter({ t }: ContactsRouterDependencies) {
  return t.router({
    list: t.procedure
      .input(zContactsListQuery.partial().optional())
    ...
    create: t.procedure.input(zContactsCreateBody).mutation(async ({ ctx, input }) => {
      ...
      if (result.isErr()) {
        return throwTrpcError(result.error)
      }
